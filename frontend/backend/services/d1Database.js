...
async function getStudyMaterialById(id) {
  initD1Schema();
  try {
    const row = db.prepare('SELECT * FROM study_materials WHERE id = ?').get(String(id));
    if (!row) return null;
    return {
      ...row,
      id: String(row.id),
...
async function deleteStudyMaterial(id) {
  ...
  try {
    ...
    console.log(`[MATERIAL DELETE] materialId=${id} d1Delete=true file_key=${existing.file_key}`);
    return existing;
  } catch (err) {
