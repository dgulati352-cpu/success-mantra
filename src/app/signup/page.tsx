...
const signupSchema = z.object({
  ...
  targetGoal: z.enum(["CBSE Commerce (Without Maths)", "CA / CS Foundation Prep", "CUET Commerce Specialist"]),
});
...
export default function SignupPage() {
  ...
  const {
    ...
    defaultValues: {
      ...
      targetGoal: "CBSE Commerce (Without Maths)",
    },
  });
  ...
  const onSubmit = async (data: SignupFormValues) => {
    try {
      setErrorMsg("");
      await signup(data.name, data.email, data.password, data.targetClass);
      router.push("/dashboard");
    } catch (err: any) {
      setErrorMsg(err.message || "Failed to create Firebase account");
    }
  };
      ...
      <div className="max-w-md w-full bg-white rounded-3xl shadow-xl border border-slate-200/80 overflow-hidden">
        ...
        <div className="p-6 sm:p-8 space-y-5">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            {/* Target Exam Goal */}
            <div className="space-y-1">
              <label className="block text-xs font-bold text-slate-700">Target Goal / Exam</label>
              <select
                {...register("targetGoal")}
              ...
              >
                ...
                <option value="CA / CS Foundation Prep">CA / CS Foundation Prep</option>
                <option value="CUET Commerce Specialist">CUET Commerce Domain Specialist</option>
              </select>
            </div>
            ...
            >
              <span>Create Profile with Firebase</span>
          ...
          <div className="relative flex py-1 items-center">
            ...
            <span className="flex-shrink mx-3 text-[10px] font-bold text-slate-400 uppercase">Or sign up with</span>
            <div className="flex-grow border-t border-slate-200" />
          </div>
