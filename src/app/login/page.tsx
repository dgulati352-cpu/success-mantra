...
import { GraduationCap, Mail, Lock, ArrowRight, UserCheck, Sparkles, KeyRound } from "lucide-react";
...
export default function LoginPage() {
  ...
  const { login, loginWithGoogle, loginAnonymouslyUser } = useAuth();
  ...
  const fillTestCredentials = () => {
    setValue("email", "student.commerce@eduprime.com");
    setValue("password", "commerce123");
  };
    ...
    <div className="min-h-[calc(100vh-4rem)] flex items-center justify-center p-4 sm:p-6 bg-slate-50">
      <div className="max-w-md w-full bg-white rounded-3xl shadow-xl border border-slate-200/80 overflow-hidden">
        ...
        <div className="p-6 sm:p-8 space-y-5">
          ...
          {/* Fill Test Credentials Shortcut */}
          <div className="flex items-center justify-between p-3 bg-blue-50/70 border border-blue-100 rounded-xl">
            <div className="flex items-center space-x-2">
              <KeyRound className="w-4 h-4 text-blue-600" />
              <span className="text-[11px] font-semibold text-blue-900">Demo Account Available</span>
            </div>
            <button
              type="button"
              onClick={fillTestCredentials}
              className="px-2.5 py-1 bg-blue-600 hover:bg-blue-700 text-white font-bold text-[10px] rounded-lg transition shadow-2xs"
            >
          ...
          <div className="relative flex py-1 items-center">
            ...
            <span className="flex-shrink mx-3 text-[10px] font-bold text-slate-400 uppercase">Or sign in with</span>
            <div className="flex-grow border-t border-slate-200" />
          </div>
          ...
          <div className="space-y-2">
            ...
            {/* Guest 1-Click Demo Login Option */}
            <button
              type="button"
              onClick={handleGuestLogin}
              className="w-full py-2.5 px-4 bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold text-xs rounded-xl flex items-center justify-center space-x-2 transition border border-slate-200"
            >
              <UserCheck className="w-4 h-4 text-emerald-600" />
              <span>Instant Guest Demo Login (1-Click)</span>
            </button>
          </div>
        </div>
      </div>
