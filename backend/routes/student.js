...
async function isStudentEnrolledInCourse(userId, courseId, reqUser = null) {
  ...
  // 3. Firestore enrollments collection
  try {
    const allEnrs = await queryCollection('enrollments');
    const matched = allEnrs.find(enr => {
      const enrCId = String(enr.course_id || enr.courseId || enr.course?.id || '');
      if (enrCId !== cId) return false;
      const enrUId = String(enr.user_id || enr.userId || enr.student?.id || '');
      const enrEmail = String(enr.student?.email || enr.email || enr.user_email || '').toLowerCase().trim();
      const isActive = !enr.status || enr.status === 'active' || enr.status === 'completed';
      if (!isActive) return false;
      return (enrUId && enrUId === uId) || (uEmail && enrEmail && enrEmail === uEmail);
    });
    if (matched) {
      return { enrolled: true, source: 'firestore_enrollment', enrollment: matched };
    }
  } catch (e) {}
...
router.get('/dashboard', async (req, res) => {
  ...
  try {
    ...
    // 1. Enrolled courses strictly belonging to student's authorized classes or direct student enrollments
    let rawEnrollments = [];
    const userEmail = (req.user?.email || '').toLowerCase().trim();
    try {
      const allEnrs = await queryCollection('enrollments');
      rawEnrollments = allEnrs.filter(enr => {
        const enrUId = String(enr.user_id || enr.userId || enr.student?.id || '');
        const enrEmail = String(enr.student?.email || enr.email || enr.user_email || '').toLowerCase().trim();
        const isActive = !enr.status || enr.status === 'active' || enr.status === 'completed';
        if (!isActive) return false;
        return (enrUId && enrUId === String(userId)) || (userEmail && enrEmail && enrEmail === userEmail);
      });
    } catch (e) {}
    ...
    for (const enrollment of rawEnrollments) {
      const cId = enrollment.course_id || enrollment.courseId || (enrollment.course && enrollment.course.id);
      ...
      if (course) {
        ...
        enrolledCourses.push({
          ...
          enrolled_at: enrollment.created_at || enrollment.enrollmentDate,
          faculty_name
        });
      }
    }
...
router.get('/courses', async (req, res) => {
  ...
  try {
    ...
    // 1. Check SQLite course_enrollments
    try {
      ...
      `).all(userId, userId);
      rows.forEach(r => {
        const cId = String(r.course_id || r.id);
        enrollmentsMap.set(cId, r);
      });
    } catch (e) {}

    // 2. Check Firestore enrollments collection
    try {
      const allEnrs = await queryCollection('enrollments');
      const studentEnrs = allEnrs.filter(enr => {
        const enrUId = String(enr.user_id || enr.userId || enr.student?.id || '');
        const enrEmail = String(enr.student?.email || enr.email || enr.user_email || '').toLowerCase().trim();
        const isActive = !enr.status || enr.status === 'active' || enr.status === 'completed';
        if (!isActive) return false;
        return (enrUId && enrUId === userId) || (userEmail && enrEmail && enrEmail === userEmail);
      });

      for (const enr of studentEnrs) {
        const cId = String(enr.course_id || enr.courseId || (enr.course && enr.course.id));
        if (!cId || enrollmentsMap.has(cId)) continue;
        const c = (await getDoc('courses', cId)) || {};
        enrollmentsMap.set(cId, {
          id: enr.id || `enr_${cId}`,
          user_id: userId,
          course_id: cId,
          enrolled_via: enr.enrolled_via || 'manual',
          status: enr.status || 'active',
          progress_percentage: enr.progress_percentage || 0,
          enrolled_at: enr.enrollmentDate || enr.created_at || new Date().toISOString(),
          title: c.title || enr.course_title || enr.course?.title || 'Enrolled Course',
          slug: c.slug || '',
          subject: c.subject || enr.subject || 'Commerce',
          target_class: c.target_class || enr.target_class || 'Class 12',
          thumbnail_url: c.thumbnail_url || 'https://images.unsplash.com/photo-1554224155-8d04cb21cd6c?w=600',
          short_description: c.short_description || c.description || '',
          instructor_name: c.instructor_name || c.faculty_name || 'CA Manish Kalra',
          price: c.price || 0,
          badge: c.badge || '',
          course_status: c.status || 'published'
        });
      }
    } catch (fsErr) {
      console.warn('Firestore enrollments query note:', fsErr);
    }

    // 3. VIP Membership or Admin / Staff access
    const mem = await checkStudentMembership(userId, req.user);
    if (mem.isMember || req.user.role === 'admin' || req.user.role === 'super_admin') {
      try {
        let allPubCourses = [];
        try {
          allPubCourses = db.prepare(`
            SELECT *, 'active' as status, 0 as progress_percentage, CURRENT_TIMESTAMP as enrolled_at, 'vip_membership' as enrolled_via
            FROM courses WHERE status = 'published' OR is_published = 1
          `).all();
        } catch (e) {}
        if (!allPubCourses.length) {
          allPubCourses = await queryCollection('courses');
        }
        for (const pub of allPubCourses) {
          const cId = String(pub.id);
          if (!enrollmentsMap.has(cId)) {
            enrollmentsMap.set(cId, {
              id: `vip_${pub.id}`,
              user_id: userId,
              course_id: pub.id,
              ...
              status: 'active',
              progress_percentage: 0,
              enrolled_at: pub.created_at || new Date().toISOString(),
              title: pub.title,
              slug: pub.slug,
              subject: pub.subject,
              target_class: pub.target_class,
              thumbnail_url: pub.thumbnail_url || 'https://images.unsplash.com/photo-1554224155-8d04cb21cd6c?w=600',
              short_description: pub.short_description,
              instructor_name: pub.instructor_name || 'CA Manish Kalra',
              price: pub.price,
              badge: pub.badge,
    ...
    const courses = Array.from(enrollmentsMap.values()).map(enr => ({
      ...
      thumbnail_url: enr.thumbnail_url || 'https://images.unsplash.com/photo-1554224155-8d04cb21cd6c?w=600',
      short_description: enr.short_description,
      instructor_name: enr.instructor_name || 'CA Manish Kalra',
      faculty_name: enr.instructor_name || enr.faculty_name || 'CA Manish Kalra',
      progress_percentage: enr.progress_percentage || 0,
...
router.get('/courses/:id', async (req, res) => {
  ...
  try {
    ...
    const cIdStr = String(course.id || courseId);

    // 1. Gather all Chapters (SQLite + Firestore top-level + Firestore subcollection)
    let chaptersMap = new Map();
    try {
      const sqlChaps = db.prepare(`
        SELECT * FROM chapters 
        WHERE course_id = ? OR course_id = CAST(? AS TEXT)
        ORDER BY order_index ASC, id ASC
      `).all(course.id, course.id);
      sqlChaps.forEach(ch => chaptersMap.set(String(ch.id), ch));
    } catch (e) {}

    try {
      const fsChaps = await queryCollection('chapters');
      fsChaps.filter(ch => String(ch.course_id) === cIdStr || String(ch.course_id) === String(courseId)).forEach(ch => {
        const k = String(ch.id);
        if (!chaptersMap.has(k)) chaptersMap.set(k, ch);
      });
    } catch (e) {}

    try {
      const subChaps = await querySubcollection('courses', cIdStr, 'chapters');
      subChaps.forEach(ch => {
        const k = String(ch.id);
        if (!chaptersMap.has(k)) chaptersMap.set(k, ch);
      });
    ...
    const chapterIdSet = new Set(chapters.map(c => String(c.id)));

    // 2. Gather all Lessons & Videos (SQLite + Firestore lessons + videos + subcollections + recordings)
    let lessonsMap = new Map();
    try {
      const sqlLessons = db.prepare(`
      ...
      sqlLessons.forEach(l => lessonsMap.set(String(l.id), l));
    } catch (e) {}

    try {
      const fsLessons = await queryCollection('lessons');
      fsLessons.filter(l => {
        return String(l.course_id) === cIdStr || String(l.course_id) === String(courseId) || (l.chapter_id && chapterIdSet.has(String(l.chapter_id)));
      }).forEach(l => {
        const k = String(l.id);
        if (!lessonsMap.has(k)) lessonsMap.set(k, l);
      });
    } catch (e) {}

    try {
      const fsVideos = await queryCollection('videos');
      fsVideos.filter(v => {
        return String(v.course_id) === cIdStr || String(v.course_id) === String(courseId) || (v.chapter_id && chapterIdSet.has(String(v.chapter_id)));
      }).forEach(v => {
        const k = String(v.id);
        if (!lessonsMap.has(k)) lessonsMap.set(k, v);
      });
    } catch (e) {}

    try {
      const subVideos = await querySubcollection('courses', cIdStr, 'videos');
      subVideos.forEach(v => {
        const k = String(v.id);
        if (!lessonsMap.has(k)) lessonsMap.set(k, v);
      });
    ...
    // Include relevant recordings matching this course or subject/class
    try {
      const fsRecs = await queryCollection('recordings');
      fsRecs.forEach(r => {
        const matchCourse = r.course_id && (String(r.course_id) === cIdStr || String(r.course_id) === String(courseId));
        const matchSubject = course.subject && r.subject && r.subject.toLowerCase().includes(course.subject.toLowerCase().slice(0, 4));
        const matchClass = (!course.target_class || !r.target_class || r.target_class.toLowerCase() === course.target_class.toLowerCase());
        if (matchCourse || (matchSubject && matchClass && r.video_url)) {
          const k = String(r.id);
          if (!lessonsMap.has(k)) {
            lessonsMap.set(k, {
              id: r.id,
              course_id: cIdStr,
              chapter_id: r.chapter_id || (chapters[0]?.id || null),
              title: r.title || 'Recorded Lecture Session',
              description: r.description || 'Live classroom recorded lecture',
              duration_minutes: r.duration_minutes || 45,
              source: 'upload',
              thumbnail_url: r.thumbnail_url || course.thumbnail_url,
    ...
    // 3. Gather all Materials (SQLite + Firestore materials + subcollection)
    let materialsMap = new Map();
    try {
      const sqlMats = db.prepare(`
      ...
      `).all(course.id, course.id);
      sqlMats.forEach(m => materialsMap.set(String(m.id), m));
    } catch (e) {}

    try {
      const fsMats = (await queryCollection('materials')).concat(await queryCollection('study_materials'));
      fsMats.forEach(m => {
        const matchCourse = m.course_id && (String(m.course_id) === cIdStr || String(m.course_id) === String(courseId));
        const matchSubject = course.subject && m.subject && m.subject.toLowerCase().includes(course.subject.toLowerCase().slice(0, 4));
        const matchClass = (!course.target_class || !m.target_class || m.target_class.toLowerCase() === course.target_class.toLowerCase());
        if (matchCourse || (matchSubject && matchClass && (m.file_url || m.pdf_url))) {
          const k = String(m.id);
          if (!materialsMap.has(k)) {
            materialsMap.set(k, {
              id: m.id,
              course_id: cIdStr,
              chapter_id: m.chapter_id || null,
              title: m.title || 'Course Study Material & Notes',
              description: m.description || '',
              file_type: m.file_type || 'PDF',
              file_size: m.file_size || '3.5 MB',
              file_url: m.file_url || m.pdf_url,
    ...
    try {
      const subMats = await querySubcollection('courses', cIdStr, 'materials');
      subMats.forEach(m => {
        const k = String(m.id);
        if (!materialsMap.has(k)) materialsMap.set(k, m);
      });
    } catch (e) {}

    const allMaterials = Array.from(materialsMap.values());

    // 4. If no chapters exist but we have lessons or materials, generate an automatic chapter
    let effectiveChapters = [...chapters];
    if (effectiveChapters.length === 0 && (allLessons.length > 0 || allMaterials.length > 0)) {
      effectiveChapters.push({
        id: 'chap_auto_1',
        title: 'Chapter 1: Lectures & Video Classes',
        description: 'Core course curriculum lectures and revision sessions',
        order_index: 1
      });
    }

    const formattedChapters = effectiveChapters.map(ch => {
      const chLessons = allLessons.filter(l => {
        if (ch.id === 'chap_auto_1') return true;
        return String(l.chapter_id) === String(ch.id);
      }).map(l => {
      ...
      const chMaterials = allMaterials.filter(m => {
        if (ch.id === 'chap_auto_1') return true;
        return String(m.chapter_id) === String(ch.id);
      }).map(m => {
...
router.get('/lessons/:id', async (req, res) => {
  ...
  try {
    ...
    if (!lesson) {
      // Check top-level Firestore collections
      try {
        const fsLessons = await queryCollection('lessons');
        lesson = fsLessons.find(l => String(l.id) === String(lessonId));
      } catch (e) {}
    }
    ...
    if (!lesson) {
      try {
        const fsVideos = await queryCollection('videos');
        lesson = fsVideos.find(v => String(v.id) === String(lessonId));
      } catch (e) {}
    }
    ...
    if (!lesson) {
      try {
        const fsRecs = await queryCollection('recordings');
        lesson = fsRecs.find(r => String(r.id) === String(lessonId));
      } catch (e) {}
    }
    ...
    const courseId = lesson.course_id || lesson.chapter_course_id || 'course_default';
    let course = null;
    try {
      course = db.prepare('SELECT * FROM courses WHERE id = ? OR id = CAST(? AS TEXT) OR id = CAST(? AS INTEGER)').get(courseId, courseId, courseId);
    } catch (e) {}
    if (!course) {
      course = (await getDoc('courses', courseId)) || { id: courseId, title: 'Course' };
    }
    ...
    if (!lesson.is_free_preview) {
      ...
      if (!enrolled) {
        return res.status(403).json({
          success: false,
          message: 'Access denied: Please enroll in this course to access paid lessons.',
          is_locked: true
        });
      }
    }
